        ...
        if (!empty($_POST)) {
            ...
            try {
                ...
                for ($i = 1; $i <= $numReg; $i++) {
                    ...
                    $respaldo = ORM::factory('Respaldo', $this->request->post('informacion_id_' . $i));
                    if ($respaldo->id) {
                        if ($respaldo->estado == 'A') {
                            $numArc = arr::get($this->request->post(), 'counter_' . $i);
                            for ($j = 1; $j <= $numArc; $j++) {
                                $archivo = ORM::factory('ArchivoRespaldo', $this->request->post('archivo_id_' . $i . '_' . $j));
                                if (!$archivo->id) {
                                    $archivo->autor = $this->request->post('autor_' . $i . '_' . $j);
                        ...
                        for ($j = 1; $j <= $numArc; $j++) {
                            ...
                            if ($archivo->archivo != null || $archivo->archivo != '') {
                                ...
                                $innactive = ORM::factory('Respaldo' );
                                $innactive=$innactive->where('id', '!=', $archivo->id)->find_all();
                                foreach ($innactive as $item) {
                                    $item->estado='I';
                                    $item->update();
                                }
                            }
