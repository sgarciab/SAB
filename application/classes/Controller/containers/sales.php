 ...
 * 
 */
class Controller_Containers_Sales extends Controller_Template {
	 ...
	 * @var String
	 */
	public $template = 'containers/sales';
	public $view;
	public $params = array();
        public $current_user;
	/**
...
// Verify if the user has access permission to the requested url
		if($this->request->controller() != "main" && $this->request->controller() != "login" && $this->request->controller() != "rpc" &&
				! $this->current_user->has_access_permission_sales($this->request->controller(), $this->request->action()))
		...
		if ( $this->auto_render )
  	    {
			...
			$this->template->title = 'MANUFACTURAS DELTA';
	...
	{		
		if ( $this->auto_render )
		{
			...
			$css_file = 'media/css/controllers/sales/'.$controller.'/'.$action.'.css';
			$styles = array(
			...
			$scripts = array(
				...
				'media/js/globalfunctions.js',
                                ...
                                'media/js/controller/sales/sale/select_rolls.js',
